...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
	 ...
	 * ResponseEntity<?> tratarEntidadeNaoEncontradaException(
	 ...
	 * tratarEntidadeEmUsoException(EntidadeEmUsoException e) { Problema problema =
	 ...
	 * tratarNegocioException(NegocioException e) { Problema problema =
	 * Problema.builder() .dataHora(LocalDateTime.now())
	 * .mensagem(e.getMessage()).build();
	 * 
    ...
    // Esse forma nova retorna   handleExceptionInternal que já vem com 
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
	    ...
	    // Foi passado um objeto para que possa ser pego seu nome.
	    List<Problem.Object> problemObjects = bindingResult.getAllErrors().stream()
	    		.map(objectError -> {
	    			String message = messageSource.getMessage(objectError, LocaleContextHolder.getLocale());
	...
	protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex,
		...
		// A biblioteca no pow CommonsLang permitiu adicionar o método abaixo para conseguirmos saber a causa raiz
	...
	private ResponseEntity<Object> handleInvalidFormat(InvalidFormatException ex, HttpHeaders headers,
		...
		// Abaixo foi feito um map para pegar uma referencia do nome do campo e reduzindo depois com colect
		// e juntando com .
		String path = joinPath(ex.getPath());
	...
	public ResponseEntity<?> handleEntidadeNaoEncontrada(
		...
		// Aqui foi gerado o objeto através do build e além disso podemos adicionar mais propriedades.
		Problem problem = createProblemBuilder(status, problemType, detail)
				.userMessage(detail)
				.build();
	...
	private Problem.ProblemBuilder createProblemBuilder(HttpStatus status, ProblemType problemType, String detail){
		...
		// Não foi colocado o buid() abaixo para que não fosse criado uma instância.
